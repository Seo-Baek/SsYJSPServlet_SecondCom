...
public class AdderServlet extends HttpServlet {
     ...
     * 2. 그런 다음사용자의 요청이나 응답에 대한 HttpServletRequest객체와
     * 	  HttpServletResponse 객체를 맏늘게 됨.
     * 3. 그리고 난 후 Servlet의 doGet() 메소드나 doPost() 메소드를 호출하면서 이 객체들을 전달.
     * 4. 톰캣이 사용자의 요청에 대한 정보를 모든 HttpServletRequest 객체의 속성으로 담아 메소드로 전달 함.
     *    따라서  각  HttpServletRequest에서 제공하는 메소드들은
     *    매개변수로 넘어온 객체들을 이용하여 사용자가 전송한 데이터를 받아오거나 응답을 할 수 있는 것임.
     *    
     * 서블릿에서 클라이언트의 요청을 얻는 방법
     * - HttpServletRequest 클래스에서 <form> 태그로 전송된 데이터를 받아오는데 사용하는 메소드.
     * 		*getParameter(String name) => <form>태그의 name 속성에 들어간 변수명을 받아서 사용.
     * 									    반황형은 String 타입임.
     *  	*getParameterValue(String name) => 같은 name에 대하여 여러 개의 값을 얻을 때 사용함.
     *  			            		             반환형은 String[] 타입임.
     *  
     *  서블릿에서 요청받은 내용을 처리하여 클라이언트에 보내는 방법
     *  1. HttpServletResponse 클래스를 이용하여 응답을 함.
     *  2. doGet()이나 doPost() 메소드 안에서 처리함.
     *  3. javax.serVlet.http.HttpServletResponse 객체를 이용함.
     *  4. setContentType() 메소드를 이용하여 클라이언트에게 전송할 데이터의 종류(MIME-TYPE)을 지정.
     *  5. 클라이언트(웹 브라우저)와 서블릿의 통신은 자바 I/O의 스트림을 이용함.
     *  
     *  웹 브라우저에서 서블릿으로 데이터를 전송하는 전송방식(2가지)
     *  1. get 방식
     *  	- 서블릿에 데이터를 전송할 때는 데이터가 url 뒤에 name=value형태로 전송이 됨.
     *  	- 여러 개의 데이터를 전송할 때는 '&'로 구분하여 전송이 됨.
     *  	- 보안이 취약함.
     *  	- 전송할 수 있는 데이터는 최대 255자.
     *  	- 기본 전송 방식이고 사용이 쉬움.
     *  	- 웹 브라우저에 직접 입력해서 전송할 수도 있음.
     *  	- 서블릿에서는 doGet() 메소드에서 전송된 데이터를 처리함.
     *  
     *  2. post 방식
     ...
     *  	- 전송 데이터의 용량이 무제한임.
     *  	- 처리 속도가 get 방식보다 느림.
     *  	- 서블릿에서는 doPost() 메소드에서 전송된 데이터를 처리함.
     */
